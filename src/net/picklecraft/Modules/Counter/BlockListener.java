...
public class BlockListener implements Listener {
	...
	public void onRedstonePower(BlockRedstoneEvent event) {
		if (event.getNewCurrent() > 0 && event.getOldCurrent() == 0) {
			...
			for (int y = 0; y < 2; y++) {
				for (int i = 0; i < 4; i++) {
					if (i == 0) { b = world.getBlockAt(X-1,Y+y,Z); }
					else if (i == 1) { b = world.getBlockAt(X+1,Y+y,Z); }
					else if (i == 2) { b = world.getBlockAt(X,Y+y,Z-1); }
					else if (i == 3) { b = world.getBlockAt(X,Y+y,Z+1); }
					if (b != null) {
