        ...
        List<IModule> modules = new ArrayList<IModule>();
            for (int s = 0; s < moduleList.length; s++) {
                try {
                    ...
                    PickleCraftPlugin.log.log(Level.SEVERE, "Failed to To instantiate module "+ moduleList[s]);
                } catch (ClassNotFoundException ex) {
                    PickleCraftPlugin.log.log(Level.SEVERE, "Can not find module "+ moduleList[s]);
                } catch (Exception ex) {
                    PickleCraftPlugin.log.log(Level.SEVERE, "Something bad happened while loading "+ moduleList[s] +" Error: "+ ex.getMessage());
                }
            }
        return modules;
    }
