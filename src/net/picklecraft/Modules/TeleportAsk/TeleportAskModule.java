		...
		if (command.getName().equalsIgnoreCase("tpa")){ 
			if (player != null) {
				if (PickleCraftPlugin.hasPerm(player, command.getPermission())) {
					/**
					 *  ability for playerA to send a dualrequest to playerB and playerC
					 *  playerC to playerB
					 */
					 if (args.length >= 2) {
									...
									if ((Boolean)player_C_Andbool[1] == false) {
										if ((Boolean)player_B_Andbool[1] == false) {
											if (pB != pC) {
												...
												player.sendMessage(
														plugin.getStringFromConfig("common.messages.errors.sameplayer"
														,args[0]
														,args[1])
														);
											}
					...
					if (args.length >= 1) {
						...
						if (coord.length >= 2) {
							try {
								...
								player.sendMessage(
										plugin.getStringFromConfig("common.messages.errors.parsefail"
										, "coordinates")
										);
							}
							return true;
